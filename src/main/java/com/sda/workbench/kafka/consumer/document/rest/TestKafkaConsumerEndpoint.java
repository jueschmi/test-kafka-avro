   ...
   public List<DocumentRest> checkKafkaMessages() {
      ...
      if (!result.isEmpty()) {
         ...
         List<DocumentRest> restDocs = result.stream()
                 .map(d -> odsMapper.mapToRestModel(d))
      ...
      if (producer != null) {
         ...
         switch (messagetype) {
            case "create":
               ...
               final RelevantPartner partner = RelevantPartner.newBuilder().setPartnerId(partnerSorKey).setPartnerRole("Versicherungsnehmer").build();
