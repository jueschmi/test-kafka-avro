...
public class DocumentODSMapper {
	...
	public void init() {
		...
		MapperFactory mapperFactory = new DefaultMapperFactory.Builder().mappingContextFactory(mappingContextFactory)
	...
	private void registerDocumentODSEventMapper(MapperFactory mapperFactory) {
		...
		mapperFactory.classMap(DocumentODSCreate.class, DocumentRestCreate.class)
				.customize(new DocumentODSCreateMapper()).byDefault().register();

		// DocumentODSDelete Mapping
		mapperFactory.classMap(DocumentODSDelete.class, DocumentRestDelete.class)
				.customize(new DocumentODSDeleteMapper()).byDefault().register();
	}

	public DocumentRest mapToRestModel(final DocumentODSEvent documentEvent) {
