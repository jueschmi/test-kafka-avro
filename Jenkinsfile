...
pipeline {
    ...
    stages {
        ...
        stage('Sonar Scan Sources (Publish to SonarQube)') {
            ...
            steps {
                sonarScanBranch project: 'com.sdase.test-kafka-avro', javaBaseDir: '.'
            }
        }
        /*
        ...
        stage('Sonar Scan Sources (Annotate Pull Request)') {
            ...
            steps {
                sonarScanPullRequest project: 'com.sdase.test-kafka-avro', javaBaseDir: '.'
            }
        }
        /*
        ...
        stage('Build Services Docker') {
            ...
            steps {
                buildDockerContainer containerName: dockerContainerPrefix + '/test-kafka-avro:' + imageTag, buildContext: ''
                pushDockerContainer containerName: dockerContainerPrefix + '/test-kafka-avro:' + imageTag, registry: dockerRegistry, credentials: 'bluemix-registry'
            }
        }
        /*
        ...
        stage('Publish release') {
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'sdabot-github-token', passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                    sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/SDA-SE/test-kafka-avro --tags')
                }
            }
        }
    }
